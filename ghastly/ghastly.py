...
if isinstance(element,         if )            coords = pack_cyl(sim_block, element, rough_pf)
            rough_pack += coords
        else:
            pass
...
if isinstance(element,         if )            reg_template = env.get_template("cylcore_template.txt")
            ...
            with open(reg_filename, mode='w') as f:
                f.write(reg_text)
elif isinstance(element,         elif )            reg_template = env.get_template("conecore_template.txt")
            reg_text = reg_template.render(region_name= element_name,
                                           x_c= element.x_c,
                                           y_c= element.y_c,
...
if isinstance(main_top,     if )        r_pour = 0.75 *main_top.r
elif isinstance(main_top,     elif )        r_pour = 0.75 *main_top.r_upper
...
if isinstance(element,         if )            x_list += [(element.x_c - element.r),
                       (element.x_c + element.r)]
            y_list += [(element.y_c - element.r),
                       (element.y_c + element.r)]
elif isinstance(element,         elif )            x_list += [(element.x_c - element.r_upper),
