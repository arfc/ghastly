...
#this is for the uco we deplete, which goes in the center pebble, and starts
#fresh
...
bg = openmc.Material(name='BG')
dep_file = 'openmc_models/inf_lat_dep/dep-center/iter4/i3_dep_res.h5'
mat_file = 'openmc_models/inf_lat_dep/dep-center/iter4/input_mats.xml'
bg_avg = material.avg_comp(dep_file, mat_file)
bg.set_density('g/cm3', 10.4)
bg.add_components(bg_avg, percent_type = 'ao')
bg.add_s_alpha_beta('c_Graphite')
bg.depletable = False
bg.temperature = 1159.15 #K
...
#define the cells and universe for the body trisos (depleting)

triso_bounds = [openmc.Sphere(r=r) for r in triso_r[:-1]]
dep_triso_cells = [openmc.Cell(fill=uco, region=-triso_bounds[0]),
...
#define the cells and universe for the triso particles in the corners
# not depleting
triso_cells = [openmc.Cell(fill=bg, region=-triso_bounds[0]),
               openmc.Cell(fill=buffer,
                           region=+triso_bounds[0] & -triso_bounds[1]),
               openmc.Cell(fill=pyc, 
                           region=+triso_bounds[1] & -triso_bounds[2]),
...
c1_trisos = [openmc.model.TRISO(triso_r[-1], triso_univ, center) 
...
c2_trisos = [openmc.model.TRISO(triso_r[-1], triso_univ, center) 
...
c3_trisos = [openmc.model.TRISO(triso_r[-1], triso_univ, center) 
...
c4_trisos = [openmc.model.TRISO(triso_r[-1], triso_univ, center) 
...
c5_trisos = [openmc.model.TRISO(triso_r[-1], triso_univ, center) 
...
c6_trisos = [openmc.model.TRISO(triso_r[-1], triso_univ, center) 
...
c7_trisos = [openmc.model.TRISO(triso_r[-1], triso_univ, center) 
...
c8_trisos = [openmc.model.TRISO(triso_r[-1], triso_univ, center) 
...
materials = openmc.Materials([uco, bg, buffer, pyc, sic, graphite, he])
...
#uco_volume = 1.5209
uco_volume = (4/3)*np.pi*(triso_r[0]**3)*19000
#print(uco_volume)
...
#d_steps = [258]*6
d_steps = [1]*4 + [5]*8 + [43]*5 + [86]*3 + [258]*4
